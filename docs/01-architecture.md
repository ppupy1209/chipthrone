...
- Pyth Network: 1분 간격으로 재조회(키·호출 한도 없음). 마지막 조회 시각을 `fxFetchedAt`으로 내려 화면에 표시
...
Hyperliquid 15분 캔들(`candleSnapshot`)과 **같은 순간의** Pyth USD/KRW를 써서 계산한다.
